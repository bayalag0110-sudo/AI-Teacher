...
# --- НЭВТРЭХ ХЭСЭГ (AUTHENTICATION) ---
def check_password():
    """Нууц үг зөв эсэхийг шалгах функц"""
    if "authenticated" not in st.session_state:
        st.session_state["authenticated"] = False

    if not st.session_state["authenticated"]:
        st.markdown("<h2 style='text-align: center;'>🔐 Системд нэвтрэх</h2>", unsafe_allow_html=True)
        col1, col2, col3 = st.columns([1, 1, 1])
        with col2:
            password = st.text_input("Нууц үгээ оруулна уу:", type="password")
            if st.button("Нэвтрэх"):
                if password == "admin123":  # <--- Энд хүссэн нууц үгээ тохируулж болно
...
# Хэрэв нэвтрээгүй бол доорх кодыг ажиллуулахгүй
if check_password():
