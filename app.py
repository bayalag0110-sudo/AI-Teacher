...
# --- CUSTOM CSS (Уусгалттай цэнхэр дэвсгэр) ---
st.markdown("""
    <style>
    .main { 
...
if auth_ui():
    ...
    with st.sidebar:
        ...
        portals = {"📚 Econtent": "https://econtent.edu.mn/book", "💻 Medle.mn": "https://medle.mn/", "📊 EduMap": "https://edumap.mn/"}
    ...
    if menu == "💎 Төлөвлөгч":
                ...
                if st.button("🚀 Боловсруулах"):
                    if file and tpc:
                        with st.spinner("AI агуулгыг шинжилж байна..."):
                            ...
                            prompt = f"БАТЛАВ: СУРГАЛТЫН МЕНЕЖЕР Б. НАМУУН\n\nPDF агуулгад тулгуурлан 70/30 харьцаатай төлөвлөгөө гарга.\nPDF Текст: {txt[:7000]}\nСэдэв: {tpc}"
        ...
        with col_out:
            if 'current_view' in st.session_state:
                ...
                doc = Document(); doc.add_paragraph(item['content']); bio = BytesIO(); doc.save(bio)
                st.download_button("📥 Word татах", bio.getvalue(), f"{item['topic']}.docx")
            else: st.info("👈 PDF файлаа оруулна уу.")
